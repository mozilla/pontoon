...
On the first day of each month, Pontoon takes a snapshot of every team: it measures the seven criteria described below, converts each one into points, and adds them up. The result is that month’s Community Health Score.
...
| [New signups](#new-signups) | 5 | Newly joined contributors who have submitted suggestions above a certain threshold|
...
A manager qualifies when the number of their **review actions plus their own approved translations exceeds 500** for the analyzed team. Review actions are approvals and rejections they performed on other people’s translations; self-approvals are only counted once towards the number of translations.
...
**15 points.** [Translators](users.md#user-roles) — contributors with review rights but are not managers — who did a substantial amount of work in the team during the measured window.
...
**4 points.** How many of the [key projects](#key-projects) are enabled for the team, as a share of all key projects.
