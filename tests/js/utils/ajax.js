...
test('PontoonDjangoAjax csrf', () => {
    ...
    expect(document.querySelector.mock.calls).toEqual([["input[name=csrfmiddlewaretoken]"]])
})
...
test('PontoonDjangoAjax headers', () => {
    ...
    expect(window.Headers.mock.calls).toEqual(
        [[{"Accept": "application/json", "X-CSRFToken": "crumble", "X-Requested-With": "XMLHttpRequest"}]])
})
...
test('PontoonDjangoAjax asMultipartForm', () => {
    ...
    expect(_ajax.asMultipartForm({foo: 17, bar: 73, baz: 117})).toBe(parameters)
    expect(parameters.append.mock.calls).toEqual(
        [["foo", 17], ["bar", 73], ["baz", 117]])
})
...
test('PontoonDjangoAjax isSameOrigin', () => {
    ...
    expect(_ajax.isSameOrigin('https://somwhere.else/all/together')).toBe(false)
...
test('PontoonDjangoAjax fetch', () => {
    ...
    expect(_ajax.fetch('foo.bar', {some: 'data'}, {method: 'get', other: 117})).toBe(7)
    expect(_ajax.get.mock.calls).toEqual(
    ...
    _ajax.get = jest.fn(() => 7)
    _ajax.post = jest.fn(() => 23)
    expect(_ajax.fetch('foo.bar', {some: 'data'}, {method: 'post', other: 43})).toBe(23)
    expect(_ajax.get.mock.calls).toEqual([])
    expect(_ajax.post.mock.calls).toEqual(
...
test('PontoonDjangoAjax appendParams', () => {
    ...
    expect(container.append.mock.calls).toEqual([["foo", 3], ["bar", 7]])
    container = {append: jest.fn(() => 111)}
    expect(_ajax.appendParams(container, {foo: 3, bar: [1, 3, 7]})).toBe(container)
    expect(container.append.mock.calls).toEqual(
        [["foo", 3], ["bar", 1], ["bar", 3], ["bar", 7]])
})
