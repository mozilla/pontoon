...
def test_project_commit_no_repos(project0):
    ...
    assert project0.repositories.count() == 0
...
@pytest.mark.django_db
def test_project_needs_sync(projectX, project1, locale0):
    """
    ...
    """
    resource = Resource.objects.create(project=projectX, path="resourceX.po")
    entity = Entity.objects.create(resource=resource, string="entityX")

    assert not projectX.needs_sync
    ChangedEntityLocale.objects.create(entity=entity, locale=locale0)
    assert projectX.needs_sync

    assert not project1.needs_sync
