...
def tag_factory(factory):
    ...
    Provides an tag factory function that accepts the following args:
...
def _calculate_tags(**kwargs):
    ...
    for tr, (_pk, _slug, _name) in translated_resource_tags:
        if _slug not in totals:
            # create an totals[tag] with zeros for this tag
...
def tag_test_kwargs(request, tag_matrix):
    ...
    **kwargs for testing the TagsTool against tags in the `tag_matrix`
    ...
    If a `slug` is an `int` its likewise mangled to `tag`.`slug`

    `projects` and `locales` are mangled to the corresponding
