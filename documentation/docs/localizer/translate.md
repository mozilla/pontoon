...
Translations can also be uploaded programmatically, using the [upload endpoint](../dev/api.md#write-endpoints) of Pontoon’s API. The endpoint requires a [Personal Access Token](users.md#personal-access-tokens) and is rate limited. Uploads performed through the UI are subject to the same rate limits.
