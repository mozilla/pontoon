...
def _get_monthly_locale_contributors(locales, months_ago):
    ...
    # Get all contributors in the given month, grouped by locale
    monthly_contributors = (
        actions.filter(
            created_at__month=month_date.month,
            created_at__year=month_date.year,
        )
        .values("translation__locale")
        .annotate(user_pks=ArrayAgg("performed_by", distinct=True))
    )
    ...
    all_user_pks = [pk for entry in monthly_contributors for pk in entry["user_pks"]]
    users = User.objects.filter(pk__in=all_user_pks)
    users_dict = {user.pk: user for user in users}

    for entry in monthly_contributors:
        locale_pk = entry["translation__locale"]
        new_contributors = []
        active_managers = []
        active_translators = []
        active_contributors = []

        for user_pk in entry["user_pks"]:
