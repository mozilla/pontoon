...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        def get_translation(entity, locale, plural_form=None):
            ...
            translations = orm.Translation.objects.filter(
            ...
            if len(translations) > 0:
                try:
                    ...
                    return t
                except orm.Translation.DoesNotExist:
                    latest = translations.order_by("date").reverse()[0]
                    return latest
            else:
                return orm.Translation()

        for r in orm.Resource.objects.all():
          entities = orm.Entity.objects.filter(resource=r)

          for l in r.project.locales.all():
            translated = approved = fuzzy = 0

            for e in entities:
              translation_array = []
              entity_translated = entity_approved = entity_fuzzy = 0

              if e.string_plural == "":
                translation = get_translation(entity=e, locale=l)
                translation_array.append(translation)
              else:
                for j in range(0, l.nplurals or 1):
                  translation = get_translation(
                    entity=e, locale=l, plural_form=j)
                  translation_array.append(translation)
              ...
              if i+1 == entity_approved:
                approved += 1
              elif i+1 == entity_fuzzy:
                fuzzy += 1
              elif i+1 == entity_translated:
                translated += 1

            s = orm.Stats(resource=r, locale=l, translated_count=translated,
                      approved_count=approved, fuzzy_count=fuzzy)
            s.save()
    ...
    models = {
        ...
        'base.resource': {
            ...
            'entity_count': ('django.db.models.fields.PositiveIntegerField', [], {}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        'base.stats': {
            ...
            'approved_count': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True', 'blank': 'True'}),
            'fuzzy_count': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True', 'blank': 'True'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'locale': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['base.Locale']"}),
            'resource': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['base.Resource']"}),
            'translated_count': ('django.db.models.fields.PositiveIntegerField', [], {'null': 'True', 'blank': 'True'})
        },
        'base.subpage': {
