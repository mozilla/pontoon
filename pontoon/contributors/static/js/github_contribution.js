  ...
  $.fn.github_graph = function (options) {
    ...
    var start = function () {
      ...
      for (
        ...
        item_html +=
          '<rect class="day" width="10" height="10" y="' +
          y +
          '" fill="' +
          color +
          '" data-count="' +
      ...
      // Add Monday, Wenesday, Friday label
            loop_html +=  '<text text-anchor="middle" class="wday" dx="695" dy="22">{0}</text>'.formatString( settings.h_days[0] )+
                          '<text text-anchor="middle" class="wday" dx="695" dy="48">{0}</text>'.formatString( settings.h_days[1] )+
                          '<text text-anchor="middle" class="wday" dx="695" dy="74">{0}</text>'.formatString( settings.h_days[2] );

      //Fixed size for now with width= 675 and height = 110
      var wire_html =
