    ...
    def __init__(
        ...
        *,
        ...
        if changed is None and removed is None and renamed is None:
            ...
            self._changes = (changed or [], removed or [], renamed or [])
...
def test_no_changes_with_no_prev_commit():
    tree: FileTree = {
...
def test_get_checkouts(_):
    ...
    assert str(exc_info.value) == "No repository found"
    ...
    with pytest.raises(Exception) as exc_info:
        ...
        checkout_repos(Mock(Project, repositories=two_sources))
    assert str(exc_info.value) == "Multiple source repositories"
    ...
    assert str(exc_info.value) == "Multiple target repositories"
