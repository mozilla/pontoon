...
from pontoon.base.system_users import (
    get_pretranslation_authors,
    get_pretranslation_user_pks,
    get_sync_user,
)
...
def test_system_users_exist_and_are_flagged():
    ...
    for email in system_users.ALL:
        user = User.objects.get(email=email)
        assert user.profile.system_user, f"{email} is not marked as a system user"
...
def test_lookups_return_the_expected_users(sync_user, gt_user, tm_user):
    ...
    assert get_pretranslation_user_pks() == {gt_user.pk, tm_user.pk}
