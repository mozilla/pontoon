...
def test_ending_newline(get_entity_mock):
    ...
    assert run_checks(po_entity, "Original", "Translation\n") == {
        "pErrors": ["Ending newline mismatch"]
    }
    assert run_checks(po_entity, "Original\n", "Translation") == {
        "pErrors": ["Ending newline mismatch"]
    }
    assert run_checks(po_entity, "Original\n", "Translation\n") == {}
    assert run_checks(po_entity, "Original", "Translation") == {}
...
def test_empty_translations(get_entity_mock):
    ...
    assert (
        run_checks(get_entity_mock("fluent", string="key = value"), "", 'key = { "x" }')
        == {}
    )
...
def test_po_newlines(get_entity_mock):
    assert run_checks(get_entity_mock("gettext"), "", "aaa\nbbb") == {}
...
def test_ftl_parse_error(get_entity_mock):
    ...
    assert run_checks(ftl_entity, "", "key =") == {
        "pErrors": ['Expected message "key" to have a value or attributes']
    }
    assert run_checks(ftl_entity, "", "key = translation") == {}
