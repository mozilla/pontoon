...
# Bumped whenever the prompt changes, so that responses generated by an earlier
# version of the prompt are not served from the cache.
PROMPT_VERSION = 1
...
class OpenAIService:
    ...
    def get_translation(
        ...
        # Reference order is meaningful, so it is preserved rather than sorted.
        references_cache_key = json.dumps(
            [[r.get("source", ""), r.get("text", "")] for r in references]
        )
        cache_key = get_machinery_service_cache_key(
            f"openai_chatgpt_v{PROMPT_VERSION}",
            english_text,
            references_cache_key,
            characteristic,
            locale.code,
            entity_key or "",
            entity_comment or "",
