...
def test_get_pretranslations(gt_mock, locale_b, resource_a, google_translate_locale):
    entities = [
    ...
    response_a = get_pretranslations(entities[0], locale_b)
    response_b = get_pretranslations(entities[0], google_translate_locale)
    ...
    response = get_pretranslations(entities[2], locale_b)
    assert response == []

    # 100% match does not exists and locale.google_translate_code is not None.
    response = get_pretranslations(entities[2], google_translate_locale)
    assert response == [("gt_translation", None, gt_user)]

    # Entity.string_plural is not None.
    response_a = get_pretranslations(entities[1], google_translate_locale)
    response_b = get_pretranslations(entities[3], google_translate_locale)
    assert response_a == [
