...
def test_mgr_user_without_translations(translation0, user0, user1):
    """
    ...
    assert user1 not in top_contributors
...
def test_mgr_user_contributors_order(user_factory, entity_factory,
    ...
    entities = entity_factory(resource=resource0, batch=22)

    # create a list of contributors/entity for translations
    contrib_list = sum(
        [[dict(user=contributors[i])] * count
         for i, count
...
@pytest.mark.django_db
def test_mgr_user_translation_counts(user_factory, entity_factory,
                                     translation_factory, resource0, locale0):
...
@pytest.mark.django_db
def test_mgr_user_period_filters(user_factory, entity_factory,
                                 translation_factory, resource0, locale0):
