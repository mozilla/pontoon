...
import {NotificationsButton, NotificationsMenu} from 'widgets/menus/notifications';
...
test('MenuItem render', () => {
    ...
    expect(listitem.props()).toEqual(
        {"className": "menu-item horizontal-separator"});
...
test('Notification render', () => {
    ...
    let notification = shallow(<MockNotification />);
    expect(notification.text()).toBe("VERBDESCRIPTION");

    notification = shallow(
        <MockNotification
           target="TITLE"
           actor="ACTOR"
           ago="TIME"  />);
...
test('Notification renderActor', () => {
    ...
    const actor = {url: 7, anchor: 'ANCHOR'};
    expect(notification.instance().renderActor(actor)).toEqual(
...
test('Notification renderDescription', () => {
    const notification = shallow(<Notification />);
    expect(notification.instance().renderDescription('DESCRIPTION')).toEqual(
        <SanitizedHTML className="message" html="DESCRIPTION" />);
});


test('Notification renderTarget', () => {
    const notification = shallow(<Notification />);
    expect(notification.instance().renderTarget({url: 'URL', name: 'TARGET'})).toEqual(
...
test('Notification renderTime', () => {
    const notification = shallow(<Notification />);
    const time = shallow(notification.instance().renderTime('AGO'));
...
test('Notification renderVerb', () => {
    const notification = shallow(<Notification />);
    expect(notification.instance().renderVerb('VERB')).toEqual(
...
test('NotificationsMenu renderNotification', () => {
    ...
    let notification = menu.instance().renderNotification({foo: "7", bar: "23"});
    expect(notification.props).toEqual({foo: "7", bar: "23"});
