#!/bin/bash
echo "out/logs is going into '/app/run_server.log'"

#user pontoon stuff --> get enc vars
(echo ">>> running as user: " && whoami) >> /app/run_server.log
if [ ! -z "$SSH_KEY" ]; then
    echo ">>> loading ssh key and kown_hosts for default user pontoon..." >> /app/run_server.log
    ...
    echo "...done." >> /app/run_server.log
fi

echo ">>> Setting up the db for Django" >> /app/run_server.log
python manage.py migrate >> /app/run_server.log

echo ">>> Starting frontend build process in the background" >> /app/run_server.log
...
echo ">>> starting continuos syncing projects" >> /app/run_server.log
/app/docker/syncprojects.sh &

echo ">>> Starting local server as user pontoon" >> /app/run_server.log
