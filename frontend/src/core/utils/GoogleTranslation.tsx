...
import xmlEntity from '../placeable/parsers/xmlEntity';
import xmlTag from '../placeable/parsers/xmlTag';
import { rules } from '../placeable';
import punctuation from '../placeable/parsers/punctuation';
...
function getRulesBasedOnInputFormat(rules: Array<Parser>): Array<Parser> {
    ...
    // Don't detect XML tags/entities
...
/**
 * Create a label for a placeable, based on its index from the placeables map.
 * Additionally, encode information about the surrounding spaces.
 */
export function GetPlaceableLabel(
    index: string,
    leftSpace: boolean,
    rightSpace: boolean,
): string {
...
/**
 * Replace the placeables with their encoded representations in order to avoid breaking them by the Google Translate API.
...
export function GetGoogleTranslateInputText(
    ...
    const placeablesRegex = new RegExp(
        '(' + placeables.map(escapeRegExp).join('|') + ')',
        'gi',
    );
    ...
    while (placeableMatch) {
        ...
        newText +=
            ...
            (leftSpace ? '' : ' ') +
            GetPlaceableLabel(
                placeablesMap.get(placeableMatch[0]),
                leftSpace,
                rightSpace,
            ) +
            (rightSpace ? '' : ' ');

        placeableMatch = placeablesRegex.exec(text);
    }
...
export function GoogleValidatePlaceables(
    ...
    return Array.from(placeablesMap.keys()).every(
        (placeable: string) => text.indexOf(placeable) !== -1,
    );
}
...
export function GetGoogleTranslateResponseText(
    ...
    while (placeableMatch) {
        let textBefore: string = text.substring(pos, placeableMatch.index),
            leftSpace: boolean = placeableMatch.groups.leftSpace === '1',
            rightSpace: boolean = placeableMatch.groups.rightSpace === '1';
        pos = placeableMatch.index + placeableMatch[0].length;

        if (!inversePlaceablesMap.has(placeableMatch.groups.placeableIndex)) {
            throw new Error(
                `Invalid placeable label: ${placeableMatch.groups.placeableIndex}`,
            );
        }
        ...
        if (
            leftSpace &&
            placeableMatch.index > 0 &&
            text[placeableMatch.index - 1] !== ' ' &&
            !isPunctuationCharacter(text[placeableMatch.index - 1])
        ) {
            newText += ' ';
        }

        newText += inversePlaceablesMap.get(
            placeableMatch.groups.placeableIndex,
        );
        ...
        if (
            ...
            pos < text.length &&
            text[pos] !== ' ' &&
            !isPunctuationCharacter(text[pos])
        ) {
            newText += ' ';
        }
