...
import {
    LocaleStatsTable,
    StatsTable, TableHeader} from 'widgets/tables';
import {LocaleFilter} from 'widgets/tables/stats';
...
test('StatsTable renderLatestActivity', () => {
    ...
    activity = stats.instance().renderLatestActivity(
        {original: {activity: {FOO: 7, BAR: 23}}});
    expect(activity).toEqual(<LatestActivity BAR={23} FOO={7} />);
});
...
test('StatsTable renderProgress', () => {
    ...
    chart = stats.instance().renderProgress(
        {original: {chart: {FOO: 7, BAR: 23}}});
...
test('LocaleStatsTable columns', () => {
    ...
    expect(table.instance().columnName).toEqual(
        {accessor: "name",
         ...
         Header: table.instance().renderNameHeader});
    expect(table.instance().columnCode).toEqual(
        {accessor: "code",
         resizable: false,
         width: 90,
         Cell: table.instance().renderCode,
         Header: table.instance().renderCodeHeader});
});


test('LocaleStatsTable renderCode', () => {
    let table = shallow(<LocaleStatsTable />);
    expect(table.instance().renderCode({original: {code: 'foo'}})).toEqual(
        <span className="code"><a href="#">foo</a></span>);
});
