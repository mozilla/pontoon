...
def test_placeable_json_placeholder():
    ...
    assert mark_placeables(u"Hello $USER$") == (
        u'Hello <mark class="placeable" ' 'title="JSON placeholder">$USER$</mark>'
    )

    assert mark_placeables(u"Hello $USER1$") == (
        u'Hello <mark class="placeable" ' 'title="JSON placeholder">$USER1$</mark>'
    )

    assert mark_placeables(u"Hello $FIRST_NAME$") == (
        u'Hello <mark class="placeable" ' 'title="JSON placeholder">$FIRST_NAME$</mark>'
    )

    assert mark_placeables(u"Hello $user$") == (u"Hello $user$")

    assert mark_placeables(u"Hello $USER") == (
        u'Hello $<mark class="placeable" ' 'title="Long all-caps string">USER</mark>'
    )
