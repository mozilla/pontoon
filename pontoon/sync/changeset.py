...
class ChangeSet(object):
    ...
    def send_notifications(self, new_entities):
        """
        Notify project translators if new entities have been added.
        """
        count = len(new_entities)

        if count > 0:
            log.info('Sending notifications for project {}.'.format(self.db_project))

            verb = 'updated with {} new strings'.format(count)
            ...
            log.info('Notifications for project {} sent.'.format(self.db_project))
